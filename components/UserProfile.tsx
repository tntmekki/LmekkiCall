...
const UserProfilePage: React.FC<UserProfilePageProps> = ({ user, onClose, onSave }) => {
  ...
  const handleSave = () => {
    onSave({ ...user, name: name.trim(), status: status.trim(), avatar });
  };
  ...
  const renderProfileForm = () => (
    <>
      <div className="p-6 sm:p-8 text-center">
        <h2 className="text-2xl font-bold text-slate-50 mb-6">الملف الشخصي</h2>
        ...
        <form onSubmit={(e) => { e.preventDefault(); handleSave(); }}>
          <div className="space-y-6 text-right">
            ...
            <div>
              <label htmlFor="userStatus" className="block text-sm font-medium text-slate-300 mb-2">الحالة</label>
              <input
                type="text"
                id="userStatus"
                value={status}
